    ...
    if request.method == 'POST':
        ...
        if 'id' in post_data:
            if post_data['action'] == 'delete':
                # TODO: delete member: database, images, file train
...
def train_face(request):
    ...
    if isTrain and isTrain == "true":
        if not member.is_train:
            label = member.recognize_label
            face_train.train(label)
            threshold = int(face_recognize.get_threshold(label))
        ...
        http_response = {
            'status': 'success',
            'message': 'Have taken enough 10 faces image!'
        }
    return JsonResponse(http_response)
